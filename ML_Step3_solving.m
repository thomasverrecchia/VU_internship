...
disp(['Matrix = ', matrixname, ', Experiment=', num2str(0), ', converge=', num2str(noerror_converge)]);

error_filename = ['./matrices/ML/', matrixname, '_errorML.mat'];
if exist(error_filename, 'file') < 1 % file does not exist 
        if noerror_converge <= M % if number of iteration in error-free run is <= number of experiments, then inject error in all iterations
            E1 = randi(noerror_converge, [1, noerror_converge]);
            repetitions = ceil(M / noerror_converge);
            E1 = repmat(E1, 1, repetitions);
            E1 = E1(1:M);
            E1 = E1';
        else
            E1 = randi(M,[1,noerror_converge])';
        end 
        E2 = datasample([1:N], M)';
        E = cat(2,E1,E2);
